...
class BPFunctionLibrary(unreal.BlueprintFunctionLibrary):
    @unreal.ufunction(ret=str, static=True, meta=dict(Category="Python Functions"))
    def GetInteropString():
        filename = filedialog.askopenfilename(initialdir=os.path.dirname(os.getcwd()), title="Select interop JSON file", filetypes=(("JSON", "*.json"), ("Text File", "*.txt")))

        return convert_interop_file(filename)
